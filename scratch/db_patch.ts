...
async function main() {
  ...
  // Ищем транзакцию на сумму 26 952 871
  ...
  if (refundTx) {
    // Нам нужен счет, который не связан с налогами, чтобы это были "просто деньги компании"
    // Идеально подойдет счет 9390 (Прочие операционные доходы) или 8710 (Нераспределенная прибыль прошлых лет)
    // Попробуем найти 9390, если нет - 8710, если нет - создадим 9390.
    ...
    // Если совсем ничего нет, привяжем к 0000 (Ввод остатков), так как это "старые" деньги
    if (!targetAccount) {
        targetAccount = await prisma.account.findFirst({
