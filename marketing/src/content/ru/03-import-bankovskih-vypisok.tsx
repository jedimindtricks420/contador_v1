...
const STEPS = [
  { num: 1, title: "Откройте мастер закрытия месяца", desc: "Импорт — первый шаг мастера в разделе «Закрытие»." },
  {
    ...
    desc: "1CClientBankExchange (.txt), до 5 МиБ и 1000 операций.",
  },
  {
    num: 3,
  ...
  },
  {
    ...
    desc: "Владелец или администратор может отменить необработанную загрузку, если счёт и открытые периоды не изменились.",
  },
];
  ...
  return (
    <div className="space-y-12">
      <Hero
        ...
        lead="Импорт банковской выписки 1CClientBankExchange (.txt) — первый шаг мастера закрытия месяца в Contador. Необработанную ошибочную загрузку можно отменить после проверки состояния счёта и периодов."
      />
      ...
      <section aria-labelledby="h2-formats">
        ...
        <p className="mt-3 text-sm text-gray-600">
          Поддерживается 1CClientBankExchange (.txt), до 5 МиБ и 1000 операций. Нужна выписка
          с номером счёта, периодом, начальным и конечным остатками. Банковские таблицы Excel
          сейчас не принимаются; переименование расширения не меняет формат файла.
        </p>
      ...
      <section aria-labelledby="h2-reimport">
        ...
        <p className="mt-3 text-sm text-gray-600">
          Откат импорта доступен владельцу или администратору для необработанной партии: закрытый период,
          проведённые операции или изменённое состояние счёта блокируют отмену. Исходная выписка и протокол
          сохраняются в архиве. Подключение по API ко всем банкам не поддерживается.
        </p>
      </section>
