...
const STEPS = [
  { num: 1, title: "Выгрузите выписку из банк-клиента", desc: "В формате .txt (1CClientBankExchange), .xls или .xlsx." },
...
const MISTAKES = [
  {
    ...
    cause: "Contador распознаёт только .txt, .xls и .xlsx",
    fix: "Выгрузить выписку заново из банк-клиента в одном из трёх поддерживаемых форматов",
  },
  {
    problem: "Одна и та же выписка загружена дважды",
    cause: "В периоде появляются задвоенные операции",
    fix: "Использовать откат импорта — он убирает операции именно повторной загрузки, не трогая остальные данные периода",
  },
  {
  ...
  return (
    <div className="space-y-12">
      <Hero
        ...
        lead="Перед загрузкой проверьте формат файла (.txt, .xls или .xlsx), убедитесь, что он покрывает нужный период, и сверьте итоговые суммы с данными банка — это снижает число ошибок на первом шаге мастера закрытия месяца."
      />
      ...
      <section aria-labelledby="h2-format">
        ...
        <p className="mt-3 text-sm text-gray-600">
          Contador принимает .txt (1CClientBankExchange), .xls и .xlsx — формат определяется автоматически при
          загрузке. На этом шаге нет отдельного фильтра по датам: импортируется всё, что есть в файле, поэтому
          важно заранее выгрузить из банк-клиента файл именно за нужный период.
        </p>
        <div className="mt-4">
