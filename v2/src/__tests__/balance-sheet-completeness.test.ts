 ...
 * (tracked by its own separate lines 320-380 in balance/route.ts, see the second
 * describe block below), must appear somewhere in BALANCE_NON_CASH_ASSET_CODES —
 * the single array balance/route.ts's asset-side Форма №1 lines are actually
 * built from (line010 through line310).
 *
...
import { BALANCE_NON_CASH_ASSET_CODES, BALANCE_CASH_CODES } from "@/app/api/reports/balance/route";
...
describe("balance-sheet-completeness — every 5xxx (cash group) ASSET/CONTRA_ASSET account is on a Форма №1 line 320-380", () => {
  it("has no uncovered account codes", () => {
    const codeSet = new Set(BALANCE_CASH_CODES);
  ...
  it("BALANCE_CASH_CODES has no duplicate codes", () => {
    const seen = new Set<string>();
    const duplicates = BALANCE_CASH_CODES.filter((code) => {
      if (seen.has(code)) return true;
      seen.add(code);
      return false;
    });
