...
**Товары или услуги при входящем ЭСФ (2026-07-10):**
Тип документа поступления больше не захардкожен на `GOODS_RECEIVED(_PREPAID)`.
Вид определяется `src/lib/receiptKind.ts` по категории исходного платежа:
`SUPPLIER_PAYMENT_SERVICES` → `SERVICE_RECEIVED(_PREPAID)` (Дт 9420 — расход),
остальные категории → `GOODS_RECEIVED(_PREPAID)` (Дт 2910 — склад/перепродажа).
В шаге 6 связь ищется через matched OpenItem/StagedTransaction, для UNMATCHED —
по последнему исходящему платежу тому же ИНН. В шаге подтверждения ЭСФ вручную
(`pending-invoices`, Step7EInvoices) пользователь видит селектор «Что получено»
с дефолтом из той же логики (`suggestedReceiptKind` в GET, `receiptKind` в POST).
Тесты: `receipt-kind.test.ts`.
...
- **Reopen:** переоткрытие месяца M удаляет PROFIT_TAX_ACCRUAL/REVERSAL всех
  кварталов года начиная с квартала M (см. `periods/[id]/reopen/route.ts`) —
  при повторном закрытии квартальных месяцев дельта пересчитывается заново.
