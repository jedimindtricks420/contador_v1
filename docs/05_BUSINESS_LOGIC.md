...
Порядок шаблонного проведения (локальное обновление G0, 2026-09-10; не выпущено):

1. Открыть транзакцию или использовать переданную; заблокировать Document (`FOR UPDATE`), загрузить тип, отклонить VOIDED и наличие существующих проводок
2. Заблокировать Period (`FOR NO KEY UPDATE`), проверить организацию, OPEN/lockDate и соответствие даты году/месяцу периода в Asia/Tashkent
3. Загрузить `Organization` (для `isVatPayer`, `taxRegime`, `turnoverTaxRate`)
4. Прочитать payload без создания контрагента
5. Собрать `evalPayload = { ...payload, isVatPayer, vatRate: 12 }`
6. Обработать строки шаблона:
  - Проверить `side`: только `debit` или `credit`
  ...
  - Отклонить отрицательные/неконечные суммы, более двух дробных знаков и значения от `10^18`
   - Пропустить строки с нулевой суммой
7. Отклонить пустой набор строк и дисбаланс; итоги сравниваются точно в целочисленных сотых (`BigInt`)
8. Проверить обязательного контрагента, найти/создать его по ИНН/имени; заблокировать подходящие OpenItem, разрешить автоматическое погашение только при точном совпадении суммы; записать `JournalEntry[]`
9. Если `template.opensItem === true` → создать `OpenItem` с `riskDeadline`
10. Закрыть выбранный OpenItem, записать `AuditLog`, синхронизировать налоговый календарь и установить `taxCalendarSyncStatus=OK`

Корневой PrismaClient оборачивается в транзакцию с maxWait=5 с, timeout=30 с;
переданная транзакция используется без вложенной, с ограничениями вызывающего кода.
Ошибки календаря больше не проглатываются: записи сервиса откатываются вместе.
Вызывающий код обязан пробросить ошибку и создавать Document в той же транзакции;
ранее созданный отдельно документ автоматически не удаляется (ACC-043 открыт).

Частичное погашение/переплата при наличии долга без точного совпадения суммы
отклоняются до проводок. При отсутствии долга отказ обязателен только для
`requireCloseMatch`; без контрагента закрытие по-прежнему пропускается, если
шаблон не требует его явно. Это защитное ограничение, не регистр распределений.
Налоговые платежи больше не помечают все просроченные события как DONE;
распределение платежей по налоговым обязательствам ещё не реализовано.

До утверждения политики округления сумма `0.005` и другие значения,
не представимые в `Decimal(20,2)` без округления, отклоняются. Это временная
защита, способная блокировать ранее принимавшиеся операции, а не завершение
ACC-054. Точность промежуточной арифметики и агрегатов отчётов требует отдельной работы.

### 1.2 voidDocument(documentId, tx?, userId?)

1. В транзакции заблокировать документ и период, проверить принадлежность организации и открытость периода
2. `Document.status = VOIDED`
3. Удалить `JournalEntry` по `documentId`
4. Закрыть связанные `OpenItem` (OPEN → CLOSED)
5. Восстановить погашенные этим документом OpenItem, отсоединить банковские строки, записать `AuditLog` и синхронизировать календарь; при ошибке откатить всё

Удаление JournalEntry остаётся прежним поведением; неизменяемое сторно и
версии проводок ещё не реализованы (ACC-037).

### 1.3 repostDocument(documentId, newTypeId)

Одна транзакция: `voidDocument()` → обновить `Document.typeId` →
`postDocument()` → восстановить связи банковских строк. Ошибка сохраняет исходные проводки.
...
Отсутствующая/неизвестная переменная теперь вызывает ошибку, а не подставляет
ноль. Числа NaN/Infinity и неконечные результаты арифметики запрещены.
Если поле допускает нулевое значение, оно должно быть передано явно.
`templateValidator` использует общий синтетический контекст всех известных
полей; он не заменяет отдельные серверные схемы типов документов.
