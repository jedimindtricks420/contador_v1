...
Если `closingData.fxDiff.difference ≠ 0` → создаёт `FX_DIFFERENCE` с payload
`{ fxDifference, fxAccountCode: "5210" }`. Счёт больше не хардкодится в шаблоне
(`$fxAccountCode` — динамический код из payload); этот автоматический шаг мастера
закрытия переоценивает только валютный банковский счёт 5210 — переоценка других
валютных статей (4010, 4310, 6010, 6820 в валюте) выполняется вручную через отдельный
`FX_DIFFERENCE`-документ с нужным `fxAccountCode`.
...
**Алгоритм:**
1. `net9910 = Σ(credit − debit)` по счёту 9910 за год — знак выбран так, чтобы прибыль
   соответствовала интуиции (счёт 9910 копит доходы по кредиту). Раньше использовался
   обратный знак (`Σ(debit − credit)`, прибыль при `< 0`), что уже приводило к ошибке
   реализации — исправлено согласованно в `closing.ts` (авто, декабрь) и в этом ручном
   API-эндпоинте.
...
> Строка 020: 9130 — себестоимость услуг, **проданных** клиентам. Покупка услуг у
> поставщика (`SERVICE_RECEIVED`/`SERVICE_RECEIVED_PREPAID`) пишет на 9420 (стр.060),
> а не на 9130 — иначе она завышала бы COGS и искажала валовую прибыль (стр.030).
...
| `FX_DIFFERENCE` | Курсовая разница | Дт $fxAccountCode → Кт 9540 / Дт 9620 → Кт $fxAccountCode |
...
| `YEAR_END_CLOSE` | Перенос финрезультата в прибыль | Дт 9910 → Кт 8710 (прибыль) / Дт 8710 → Кт 9910 (убыток) |
| `REFUND` | Возврат покупателю (расходная операция) | Дт 9040 → Кт 5110 |
| `SUPPLIER_REFUND` | Возврат от поставщика | Дт 5110 → Кт 4310 |
| `DIVIDEND_ACCRUAL` | Начисление дивидендов учредителям | Дт 8710 → Кт 6610 |
| `DIVIDEND_PAYMENT` | Выплата дивидендов учредителям | Дт 6610 → Кт 5110 |

Полный и всегда актуальный справочник — [v2/docs/modules/module_DOCUMENT_TYPES.md](../v2/docs/modules/module_DOCUMENT_TYPES.md)
(этот раздел и корневой `DOCUMENT_TYPES.md` — иллюстративные, не исчерпывающие).
...
**Дедлайны по умолчанию** (`RISK_DAYS_BY_ACCOUNT` / `RISK_DAYS_DEFAULT` в `constants.ts`),
могут быть переопределены на уровне организации через `Organization.settings.openItemDeadlines`
(`GET/PATCH /api/settings/open-item-deadlines` — учитывается в `getRiskDeadline()`):

| Счёт | Тип | Дедлайн |
|------|-----|---------|
| 4220 | Подотчёт (командировки) | 10 дней |
| 4230 | Подотчёт (общехозяйственные) | 10 дней |
| 4310 | Аванс поставщику | 30 дней (`RISK_DAYS_DEFAULT`) |
| 6310 | Аванс от покупателя | 30 дней (`RISK_DAYS_DEFAULT`) |
| 6990 | Неидентифицированное поступление | 30 дней (`RISK_DAYS_DEFAULT`) |
| 4890 | Депозит / расчёты с агрегатором | 365 дней |
| 6820 | Займ учредителя | 365 дней |
| 6610 | Дивиденды к оплате | 365 дней |

Список буферных счетов не хардкодится отдельно — вычисляется из всех типов документов с
`opensItem: true` (`getOpenItemBufferAccountCodes()` в `ensureBaseData.ts`).
