...
Список буферных счетов не хардкодится — вычисляется из всех типов документов с `opensItem: true` (`getOpenItemBufferAccountCodes()`), название берётся из плана счетов, `days` — override организации либо встроенный дефолт.

**Response 200:** `{ "accounts": [{ "code": "4220", "name": "Авансы, выданные на служебные командировки", "days": 10 }, ...] }`

### PATCH /settings/open-item-deadlines

**Body:** `{ "4310": 45, "6610": 180 }` — частичное обновление, только переданные коды (сливается с существующими overrides). Каждый код должен входить в список буферных счетов, каждое значение — положительное число, иначе `400`.

**Response 200:** `{ "openItemDeadlines": { "4310": 45, "6610": 180, ... } }` — полный объединённый объект overrides.
